...
NavigationHubLayout.journey()
                        ...
                        <p class="ny-tab-blurb">{{ __('Build bottom nav, top nav, or journey flows with state maintenance.') }}</p>
                        <a href="{{ route('landing.docs', ['locale' => app()->getLocale(), 'version' => $latestVersionOfNylo, 'page' => 'navigation-hub']) }}" target="_BLANK" class="ny-tab-link">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-5">
                            @foreach([
                                ...
                                <a class="ny-chip justify-center py-2.5" href="{{ route('landing.docs', ['locale' => app()->getLocale(), 'version' => $latestVersionOfNylo, 'page' => 'navigation-hub']) }}#{{ $item['link'] }}" target="_BLANK">{{ $item['title'] }}</a>
                            @endforeach
                        </div>
