...
You can either download {{ config('app.name') }} v5 <a href="https://github.com/nylo-core/nylo/archive/refs/heads/5.x.zip">here</a> or clone the git repository using the below command.

```bash
git clone -b 5.x https://github.com/nylo-core/nylo.git nylo_app
```
...
You will also need to add a `.env` file to your project at the root level. If you've downloaded {{ config('app.name') }} from GitHub, this file will already be there.
