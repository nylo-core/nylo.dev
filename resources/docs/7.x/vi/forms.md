...
- Các loại trường
  ...
  - [Trường builder](#builder-fields "Trường builder")
...
Field.mask("Custom Code",
  mask: "AA-####",
  match: r'[\w\d]',
  maskReturnValue: true, // Trả về giá trị đã định dạng
)
```
...
Sử dụng `Field.builder()` để tạo trường form inline tùy chỉnh mà không cần kế thừa `NyFieldStatefulWidget`. Hàm builder nhận giá trị hiện tại, callback `onChanged` để báo cáo thay đổi giá trị cho form, và callback `setState` để kích hoạt xây dựng lại giao diện.
  ...
  builder: (context, onChanged, value, setState) {
    return ColorPicker(
      ...
      onColorChanged: (color) {
        onChanged(color);
        setState(); // xây dựng lại widget trường
      },
    );
  },
  value: Colors.blue,
...
Tham số thứ ba là giá trị trường hiện tại và thứ tư là `setState`. Nếu builder không cần `setState`, bạn có thể dùng cú pháp 3 tham số cũ (`NyFieldBuilderLegacy`), vẫn được hỗ trợ:
...
``` dart
// Từ danh sách chuỗi (value và label giống nhau)
FormCollection.from(["Red", "Green", "Blue"])

// Giống như trên, tường minh
FormCollection.fromArray(["Red", "Green", "Blue"])

// Từ map (key = value, value = label)
FormCollection.fromMap({
  "us": "United States",
  "ca": "Canada",
})

// Từ dữ liệu có cấu trúc (hữu ích cho API response)
...
// Nullable — xác thực vượt qua khi trường trống
Field.text("Nickname",
  validator: FormValidator().minLength(3).nullable(),
)
```

`nullable()` đánh dấu validator là tùy chọn. Khi giá trị trường là null hoặc trống, tất cả quy tắc xác thực bị bỏ qua và trường vượt qua. Khi trường có giá trị, tất cả quy tắc được áp dụng bình thường. Nối nó vào cuối bất kỳ `FormValidator` nào.
...
``` dart
// Chỉ đặt tùy chọn (không có giá trị ban đầu)
"Category": define(options: categories),

// Chỉ đặt giá trị ban đầu
"Price": define(value: "100"),

// Đặt cả giá trị và tùy chọn
"Country": define(value: "us", options: countries),

// Giá trị đơn giản vẫn hoạt động với trường đơn giản
"Name": "John",
```
...
EditAccountForm(
  onSubmit: (data) {
    // data là một Map<String, dynamic>
...
// Làm mới các trường form (gọi lại fields())
...
@override
fields() => [
  // Trường đơn (toàn bộ chiều rộng)
  Field.text("Title"),

  // Hai trường trong một hàng
  [
    Field.text("First Name"),
    Field.text("Last Name"),
  ],
  ...
  // Slider và range slider trong một hàng
  [
  ...
  // Nhúng widget không phải trường
  Field.widget(child: Divider()),
...
``` dart
// Bên trong lớp con NyFormWidget hoặc callback onChanged
Field nameField = ...;
...
``` dart
// Gửi form có tên "LoginForm" từ bất kỳ đâu
NyFormWidget.submit("LoginForm", onSuccess: (data) {
  print(data);
});

// Cập nhật giá trị trường từ xa
...
LoginForm(
  Key? key,
  double crossAxisSpacing = 10,  // Khoảng cách ngang giữa các trường trong hàng
  double mainAxisSpacing = 10,   // Khoảng cách dọc giữa các trường
  Map<String, dynamic>? initialData, // Giá trị ban đầu của trường
  Function(Field field, dynamic value)? onChanged, // Callback khi trường thay đổi
  Widget? header,                // Widget phía trên form
  Widget? submitButton,          // Widget nút gửi
  Widget? footer,                // Widget phía dưới form
  double headerSpacing = 10,     // Khoảng cách sau header
  double submitButtonSpacing = 10, // Khoảng cách sau nút gửi
  double footerSpacing = 10,     // Khoảng cách trước footer
  LoadingStyle? loadingStyle,    // Kiểu chỉ báo tải
  bool locked = false,           // Làm cho form chỉ đọc
  Function(dynamic data)? onSubmit,   // Được gọi với dữ liệu form khi xác thực thành công
  Function(dynamic error)? onFailure, // Được gọi với lỗi khi xác thực thất bại
)
```
...
| `Field.builder()` | `builder` (bắt buộc `NyFieldBuilder` hoặc `NyFieldBuilderLegacy`) | Trường inline tùy chỉnh không cần kế thừa |
