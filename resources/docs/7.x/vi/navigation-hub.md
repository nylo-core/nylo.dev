...
class _BaseNavigationHubState extends NavigationHub<BaseNavigationHub> {
  ...
  /// Có nên duy trì trạng thái không
  @override
...
// hoặc điều hướng đến Navigation Hub từ bất kỳ đâu trong ứng dụng
...
@override
NavigationHubLayout? layout(BuildContext context) => NavigationHubLayout.bottomNav(
    style: BottomNavStyle.material(), // Kiểu material Flutter mặc định
);
```
...
_MyNavigationHubState() : super(() => {
    ...
    1: NavigationTab.journey(
        ...
        ), // ghi đè mặc định layout chỉ cho tab này
    ),
    2: NavigationTab.journey(
        page: AddPhotosStep(),
    ),
...
class _WelcomeState extends JourneyState<Welcome> {
  ...
  /// Kiểm tra xem journey có thể tiếp tục bước tiếp theo không
  @override
  Future<bool> canContinue() async {
    return true;
  }

  /// Được gọi trước khi điều hướng đến bước tiếp theo
  @override
  Future<void> onBeforeNext() async {
    // Ví dụ: lưu dữ liệu vào session
  }

  /// Được gọi khi journey hoàn thành (ở bước cuối)
  @override
...
Widget view(BuildContext context) {
    return buildJourneyContent(
        ...
        nextButton: Button.primary(
            text: isLastStep ? "Get Started" : "Continue",
            onPressed: nextStep, // chạy validation rồi điều hướng
        ),
    );
}
```
...
Future<void> onAfterNext() async {
    // print('Đã điều hướng đến bước tiếp theo');
}
```
...
@override
Future<bool> canContinue() async {
    // Thực hiện logic validation của bạn ở đây
    // Trả về true nếu journey có thể tiếp tục, false nếu không
...
nextButton: Button.primary(
    text: "Skip to photos",
    onPressed: () {
        goToStep(2); // nhảy đến step index 2
    },
),
```
...
``` dart
onPressed: () {
    goToNextStep(); // bỏ qua validation và đến bước tiếp theo
},
```
...
class _CompleteStepState extends JourneyState<CompleteStep> {
  ...
  /// Callback khi journey hoàn thành
  @override
  void Function()? get onJourneyComplete => () {
    // Điều hướng đến trang chủ hoặc đích tiếp theo
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Button.primary(
            text: "Get Started",
            onPressed: onJourneyComplete, // kích hoạt completion callback
          ),
        ],
      ),
    );
...
Khi pop từ navigator lồng nhau bên trong tab, sử dụng `rootNavigator: true` để pop từ root navigator thay vì navigator cục bộ của tab:

``` dart
// Pop từ navigator cục bộ của tab (mặc định)
pop();

// Pop từ root navigator — dùng khi modal hoặc overlay
// được push bằng root navigator
pop(rootNavigator: true);
```

Tham số `rootNavigator` có sẵn trên `pop()` trong `NyState`, `NyController`, `StateAction.pop()`, và extension `BuildContext`.
...
class _MyNavigationHubState extends NavigationHub<MyNavigationHub> {
    ...
    @override
    int get initialIndex => 1; // Bắt đầu ở tab thứ hai
    ...
}
```
...
class _MyNavigationHubState extends NavigationHub<MyNavigationHub> {
    ...
    onTap(int index) {
        ...
        // Ví dụ: theo dõi analytics, hiển thị xác nhận, v.v.
        super.onTap(index); // Luôn gọi super để xử lý chuyển tab
    }
}
```
...
``` dart
/// Đặt lại tab tại chỉ số cho trước
/// Ví dụ: MyNavigationHub.stateActions.resetTabIndex(0);
resetTabIndex(int tabIndex);

/// Thay đổi tab hiện tại theo chương trình
/// Ví dụ: MyNavigationHub.stateActions.currentTabIndex(2);
currentTabIndex(int tabIndex);

/// Cập nhật số badge
/// Ví dụ: MyNavigationHub.stateActions.updateBadgeCount(tab: 0, count: 2);
updateBadgeCount({required int tab, required int count});

/// Tăng số badge
/// Ví dụ: MyNavigationHub.stateActions.incrementBadgeCount(tab: 0);
incrementBadgeCount({required int tab});

/// Xóa số badge
/// Ví dụ: MyNavigationHub.stateActions.clearBadgeCount(tab: 0);
clearBadgeCount({required int tab});

/// Bật cảnh báo cho tab
/// Ví dụ: MyNavigationHub.stateActions.alertEnableTab(tab: 0);
alertEnableTab({required int tab});

/// Tắt cảnh báo cho tab
/// Ví dụ: MyNavigationHub.stateActions.alertDisableTab(tab: 0);
alertDisableTab({required int tab});

/// Điều hướng đến trang tiếp theo trong layout journey
/// Ví dụ: await MyNavigationHub.stateActions.nextPage();
Future<bool> nextPage();

/// Điều hướng đến trang trước trong layout journey
/// Ví dụ: await MyNavigationHub.stateActions.previousPage();
Future<bool> previousPage();

/// Làm mới tab cụ thể, buộc nó rebuild
/// Ví dụ: MyNavigationHub.stateActions.refreshTab(0);
void refreshTab(int tabIndex);

/// Làm mới tất cả tab, buộc chúng rebuild
/// Ví dụ: MyNavigationHub.stateActions.refresh();
void refresh();
```
...
MyNavigationHub.stateActions.currentTabIndex(2); // Chuyển sang tab 2

await MyNavigationHub.stateActions.nextPage(); // Journey: đến trang tiếp theo

MyNavigationHub.stateActions.refreshTab(0); // Buộc tab 0 rebuild

MyNavigationHub.stateActions.refresh(); // Buộc tất cả tab rebuild
```
...
class _MyNavigationHubState extends NavigationHub<MyNavigationHub> {
    ...
    _MyNavigationHubState() : super(() async {
      ...
      await sleep(3); // mô phỏng tải trong 3 giây
