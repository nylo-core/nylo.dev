...
- [Tùy chỉnh kiểu Toast](#custom-styles "Tùy chỉnh kiểu Toast")
  ...
  - [Kiểu Toast nhận dữ liệu động](#data-aware-toast-styles "Kiểu Toast nhận dữ liệu động")
...
// Toast thông tin — description là tùy chọn
showToastInfo();

// Toast nguy hiểm với thời gian hiển thị tùy chỉnh
...
Các phương thức khả dụng: `showToastSuccess`, `showToastWarning`, `showToastInfo`, `showToastDanger`, `showToastOops`, `showToastSorry`, `showToastCustom`. Tất cả đều chấp nhận các tham số tùy chọn `description`, `duration` (`Duration?`), và `data` (`Map<String, dynamic>?`).
...
showToastNotification(
  ...
  action: () {
    // Được gọi khi toast được nhấn
    routeTo("/details");
  },
  onDismiss: () {
    // Được gọi khi toast bị đóng
  },
  onShow: () {
    // Được gọi khi toast hiển thị
  },
);
```
...
| `title` | `String?` | null | Nội dung tiêu đề; được truyền nguyên vẹn vào widget toast |
| `description` | `String?` | null | Nội dung mô tả |
| `data` | `Map<String, dynamic>?` | null | Các cặp key-value truyền đến kiểu toast nhận dữ liệu động; `title` và `description` được ưu tiên hơn các key trùng khớp |
...
Đăng ký kiểu nhận dữ liệu động bằng `registerWithData()`:
...
Hoặc đăng ký cùng với các kiểu tĩnh trong `AppProvider`:
...
Gọi với map `data` tại thời điểm chạy:
...
Nếu bạn cũng truyền `title` hoặc `description`, chúng sẽ được ưu tiên hơn các key trùng khớp trong `data`.

Dùng `ToastNotificationRegistry.resolve(id, data)` trực tiếp nếu bạn cần tự tạo widget:
