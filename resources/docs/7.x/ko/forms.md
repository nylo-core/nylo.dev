...
Field.mask("Custom Code",
  mask: "AA-####",
  match: r'[\w\d]',
  maskReturnValue: true, // 포맷된 값을 반환
)
```
...
`Field.builder()`를 사용하여 `NyFieldStatefulWidget`을 서브클래싱하지 않고 커스텀 인라인 폼 필드를 만듭니다. 빌더 함수는 현재 값, 폼에 값 변경을 보고하는 `onChanged` 콜백, UI 재빌드를 트리거하는 `setState` 콜백을 받습니다.
...
세 번째 매개변수는 현재 필드 값이고 네 번째는 `setState`입니다. 빌더가 `setState`를 필요로 하지 않는 경우, 여전히 지원되는 레거시 3인수 시그니처(`NyFieldBuilderLegacy`)를 사용할 수 있습니다:
...
// 널 허용 — 필드가 비어 있으면 유효성 검사 통과
Field.text("Nickname",
  validator: FormValidator().minLength(3).nullable(),
)
```

`nullable()`은 유효성 검사기를 선택 사항으로 표시합니다. 필드 값이 null이거나 비어 있으면 모든 유효성 검사 규칙이 건너뛰어지고 필드가 통과됩니다. 필드에 값이 있으면 모든 규칙이 정상적으로 적용됩니다. 모든 `FormValidator` 끝에 체인으로 연결하세요.
...
``` dart
// NyFormWidget 서브클래스 또는 onChanged 콜백 내에서
Field nameField = ...;
...
LoginForm(
  Key? key,
  double crossAxisSpacing = 10,  // 행 필드 사이의 수평 간격
  double mainAxisSpacing = 10,   // 필드 사이의 수직 간격
  Map<String, dynamic>? initialData, // 초기 필드 값
  Function(Field field, dynamic value)? onChanged, // 필드 변경 콜백
  Widget? header,                // 폼 위의 위젯
  Widget? submitButton,          // 제출 버튼 위젯
  Widget? footer,                // 폼 아래의 위젯
  double headerSpacing = 10,     // 헤더 후 간격
  double submitButtonSpacing = 10, // 제출 버튼 후 간격
  double footerSpacing = 10,     // 푸터 전 간격
  LoadingStyle? loadingStyle,    // 로딩 인디케이터 스타일
  bool locked = false,           // 폼을 읽기 전용으로 만들기
  Function(dynamic data)? onSubmit,   // 유효성 검사 성공 시 폼 데이터와 함께 호출
  Function(dynamic error)? onFailure, // 유효성 검사 실패 시 에러와 함께 호출
)
```
