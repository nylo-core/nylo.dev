...
**단계 1:** `NyStateManaged`를 확장하는 Widget 정의
...
1. `Cart` Widget은 `StatefulWidget`을 직접 확장하는 것이 아닌 `NyStateManaged`를 확장합니다.

2. `stateName` 생성자 매개변수는 `super(child: () => _CartState(stateName))`를 통해 전달되어 같은 Widget의 여러 독립적인 인스턴스를 가능하게 합니다.

3. `_stateFor(String? state)` 헬퍼는 명명된 인스턴스에 대해 `"cart_sidebar"`와 같은 네임스페이스 상태 키를 생성합니다.

4. `_CartState`는 `NyState<Cart>`를 확장하며 올바른 독립 상태를 등록하기 위해 `stateName`을 받습니다.

5. `stateActions` 맵은 앱의 어디서나 Widget에서 호출할 수 있는 명명된 명령을 정의합니다.
