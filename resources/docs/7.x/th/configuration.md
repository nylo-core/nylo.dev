...
- สภาพแวดล้อม
  ...
  - [การแทรกค่าตัวแปร](#variable-interpolation "การแทรกค่าตัวแปร")
...
คำสั่งนี้จะเขียนทับ `env.g.dart` ที่มีอยู่เสมอ
...
ค่าสตริงในไฟล์ `.env` ของคุณสามารถอ้างอิงตัวแปรอื่นได้โดยใช้ไวยากรณ์ `${VAR_NAME}`:
...
เมื่อโค้ดของคุณเรียก `getEnv('APP_URL')` ค่าที่ส่งคืนคือ `https://myapp.com` การอ้างอิงถูกแก้ไขแบบ recursive ดังนั้นการอ้างอิงแบบลูกโซ่ก็ทำงานได้ตามที่คาดหวัง:
...
`getEnv('UPLOADS')` คืนค่า `https://example.com/uploads`

การอ้างอิงแบบวงกลมได้รับการป้องกัน — หากตัวแปรอ้างอิงตัวเอง (โดยตรงหรือผ่านลูกโซ่) placeholder `${VAR_NAME}` ที่ยังไม่ได้แก้ไขจะถูกเก็บไว้ในผลลัพธ์แทนที่จะทำให้เกิด infinite loop
...
3. **สร้างใหม่หลังแก้ไข** - รัน `metro make:env` ทุกครั้งหลังแก้ไข `.env`
