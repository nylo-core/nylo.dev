...
**ステップ 1:** `NyStateManaged` を継承するウィジェットを定義
...
1. `Cart` ウィジェットは `StatefulWidget` を直接継承するのではなく、`NyStateManaged` を継承します。

2. `stateName` コンストラクタパラメータは `super(child: () => _CartState(stateName))` を通じて転送され、同じウィジェットの複数の独立したインスタンスを有効にします。

3. `_stateFor(String? state)` ヘルパーは、名前付きインスタンスに対して `"cart_sidebar"` のような名前空間付き状態キーを生成します。

4. `_CartState` は `NyState<Cart>` を継承し、正しい独立した状態を登録するために `stateName` を受け取ります。

5. `stateActions` マップは、アプリのどこからでもウィジェットに対して呼び出せる名前付きコマンドを定義します。
