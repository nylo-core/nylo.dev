...
- [Installazione](#install "Installazione")
- Comandi Make
  - [Make controller](#make-controller "Make controller")
  - [Make model](#make-model "Make model")
  - [Make page](#make-page "Make page")
  - [Make stateless widget](#make-stateless-widget "Make stateless widget")
  - [Make stateful widget](#make-stateful-widget "Make stateful widget")
  - [Make journey widget](#make-journey-widget "Make journey widget")
  - [Make API Service](#make-api-service "Make API Service")
  - [Make Event](#make-event "Make Event")
  - [Make Provider](#make-provider "Make Provider")
  - [Make Forms](#make-forms "Make Forms")
  - [Make Route Guard](#make-route-guard "Make Route Guard")
  - [Make Config File](#make-config-file "Make Config File")
  - [Make Command](#make-command "Make Command")
  - [Make State Managed Widget](#make-state-managed-widget "Make State Managed Widget")
  - [Make Navigation Hub](#make-navigation-hub "Make Navigation Hub")
  - [Make Bottom Sheet Modal](#make-bottom-sheet-modal "Make Bottom Sheet Modal")
  - [Make Button](#make-button "Make Button")
  - [Make Interceptor](#make-interceptor "Make Interceptor")
  - [Make Env File](#make-env-file "Make Env File")
  - [Make Key](#make-key "Make Key")
- Icone dell'App
  - [Generare Icone dell'App](#build-app-icons "Generare Icone dell'App")
...
Tutti i comandi `make:*` accettano un separatore di percorso nel nome per posizionare il file in una sottodirectory:

``` bash
metro make:stateless_widget login/BrandPanel
```

Questo crea il widget in `lib/resources/widgets/login/brand_panel.dart`.
...
Usa un separatore di percorso per organizzare l'evento in una sottodirectory:

``` bash
metro make:event auth/login_event
```

Questo crea l'evento in `lib/app/events/auth/login_event.dart`.
...
Usa un separatore di percorso per organizzare il provider in una sottodirectory:

``` bash
metro make:provider integrations/firebase_provider
```

Questo crea il provider in `lib/app/providers/integrations/firebase_provider.dart`.
...
Usa un separatore di percorso per organizzare il form in una sottodirectory:

``` bash
metro make:form checkout/car_advert_form
```

Questo crea il form in `lib/app/forms/checkout/car_advert_form.dart`.
...
Usa un separatore di percorso per organizzare il guard in una sottodirectory:

``` bash
metro make:route_guard subscriptions/premium_content
```

Questo crea il guard in `lib/app/route_guards/subscriptions/premium_content.dart`.
...
Il comando precedente creera' un nuovo widget in `lib/resources/widgets/`. Il widget generato estende `NyStateManaged`, che supporta l'isolamento multi-istanza tramite il parametro costruttore `stateName`.
...
``` bash
# Genera la chiave per un file env specifico
...
CommandBuilder builder(CommandBuilder command) {
  ...
  // Aggiunge un'opzione con un valore predefinito
  command.addOption(
    'environment',     // nome dell'opzione
    abbr: 'e',         // abbreviazione
    help: 'Target deployment environment', // testo di aiuto
    defaultValue: 'development',  // valore predefinito
    allowed: ['development', 'staging', 'production'] // valori consentiti
  );
...
CommandBuilder builder(CommandBuilder command) {
  ...
  command.addFlag(
    'verbose',       // nome del flag
    abbr: 'v',       // abbreviazione
    help: 'Enable verbose output', // testo di aiuto
    defaultValue: false  // disattivato per impostazione predefinita
  );
...
```dart
// Esegui un processo con output visualizzato nella console
await runProcess('flutter build web --release');

// Esegui un processo silenziosamente
await runProcess('flutter pub get', silent: true);

// Esegui un processo in una directory specifica
...
```dart
// Aggiungi un pacchetto al pubspec.yaml
addPackage('firebase_core', version: '^2.4.0');

// Aggiungi un pacchetto dev al pubspec.yaml
addPackage('build_runner', dev: true);

// Aggiungi piu' pacchetti contemporaneamente
...
```dart
// Stampa messaggi di stato con codice colore
info('Processing files...');    // Testo blu
error('Operation failed');      // Testo rosso
success('Deployment complete'); // Testo verde
warning('Outdated package');    // Testo giallo
```
...
if (confirm('Would you like to continue?', defaultValue: true)) {
  // Utente ha confermato o premuto Invio (accettando il predefinito)
...
@override
Future<void> handle(CommandResult result) async {
  // Esegui un'attivita' con uno spinner
  final projectFiles = await withSpinner(
    task: () async {
      // Attivita' di lunga durata (es., analisi dei file del progetto)
      await sleep(2);
...
Future<void> handle(CommandResult result) async {
  ...
  try {
    // Esegui piu' passaggi con aggiornamenti di stato
...
Future<void> handle(CommandResult result) async {
  ...
  // Stampa righe vuote
  newLine();       // una riga vuota
  newLine(3);      // tre righe vuote

  // Stampa un commento attenuato (testo grigio)
  comment('This is a background note');

  // Stampa un riquadro di avviso prominente
  alert('Important: Please read carefully');

  // ask e' un alias per prompt
  final name = ask('What is your name?');

  // Input nascosto per dati sensibili (es., password, chiavi API)
  final apiKey = promptSecret('Enter your API key:');

  // Interrompi il comando con un messaggio di errore e codice di uscita
  if (name.isEmpty) {
...
Future<void> handle(CommandResult result) async {
  ...
  // Costruisci un percorso personalizzato relativo alla root del progetto
...
Future<void> handle(CommandResult result) async {
  ...
  // Leggi le variabili d'ambiente di sistema
...
@override
Future<void> handle(CommandResult result) async {
  // Formatta un file o una directory Dart
  ...
  // Compila per un target con argomenti aggiuntivi
  await flutterBuild('apk', args: ['--release', '--split-per-abi']);
  await flutterBuild('web', args: ['--release']);

  // Esegui flutter test
  await flutterTest();
  await flutterTest('test/unit/');  // directory specifica
}
```
...
@override
Future<void> handle(CommandResult result) async {
  // Aggiunge un'istruzione import a un file Dart (evita duplicati)
  await addImport(
    'lib/bootstrap/providers.dart',
    "import '/app/providers/firebase_provider.dart';",
  );

  // Inserisce codice prima dell'ultima parentesi graffa chiusa in un file
  // Utile per aggiungere voci alle mappe di registrazione
  await insertBeforeClosingBrace(
    'lib/bootstrap/providers.dart',
    '  FirebaseProvider(),',
  );

  // Verifica se un file contiene una stringa specifica
  bool hasImport = await fileContains(
    'lib/bootstrap/providers.dart',
    'firebase_provider',
  );

  // Verifica se un file corrisponde a un pattern regex
...
Future<void> handle(CommandResult result) async {
  ...
  // Trova file per pattern del nome
...
@override
Future<void> handle(CommandResult result) async {
  // Crea una barra di avanzamento per 100 elementi
...
Future<void> handle(CommandResult result) async {
  ...
  // Elabora gli elementi con tracciamento automatico del progresso
