...
Приведённая выше команда создаст новый виджет в `lib/resources/widgets/`. Сгенерированный виджет расширяет `NyStateManaged`, который поддерживает изоляцию нескольких экземпляров через параметр конструктора `id`.
